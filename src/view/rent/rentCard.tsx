...
export function RentCard({ data, onFavorite }: RentCardProps) {
  ...
  return (
    <Card
      className="group relative overflow-hidden bg-[#ececec] p-0 shadow-lg hover:shadow-xl transition-all duration-300 rounded-lg cursor-pointer border border-gray-200 h-full flex flex-col"
      ...
      <div className="relative w-full h-72 md:h-80 overflow-hidden flex-shrink-0">
        ...
        {data.price && (
          <div className="absolute bottom-4 left-4 bg-white/95 backdrop-blur-sm text-[#1A202C] text-xl font-bold px-4 py-2 rounded-lg shadow-xl flex items-center gap-2 z-20">
            ...
            <span className="text-base font-normal text-gray-600">/{data?.priceType}</span>
          </div>
        )}
